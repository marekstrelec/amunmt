...
int main() {
  ...
  FloatMatrixMultiplication(float_lhs.ConstMap(), float_rhs.ConstMap(),
    ...
    std::cout << ">>>>> Normal: " << (std::clock() - start) / (double) (CLOCKS_PER_SEC) << "ms" << std::endl;
  ...
  gemmlowp::GemmWithOutputPipeline<std::uint8_t, std::uint8_t,
    ...
    std::cout << ">>>>> Quant: " << (std::clock() - start2) / (double) (CLOCKS_PER_SEC) << "ms" << std::endl;
